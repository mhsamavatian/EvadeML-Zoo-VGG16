...
def main(argv=None):
    # 0. Select a dataset.
    from datasets import MNISTDataset, ImageNetDataset, SVHNDataset
    #CIFAR10Dataset, 
    ...
    for attack_string in attack_string_list:
        ...
        print (X_test.shape,Y_test_target.shape)
        duration=0
        """
        X_test_adv_temp_list=[]
        chunck = 20
        
        for chk in range(0,X_test.shape[0],chunck):
            #print (X_test[chk:chk+chunck].shape)
            #print (x[chk:chk+chunck].shape)
            X_test_adv_temp, aux_info = maybe_generate_adv_examples(sess, model, x[chk:chk+chunck], y[chk:chk+chunck], X_test[chk:chk+chunck], Y_test_target[chk:chk+chunck], attack_name, attack_params, use_cache = x_adv_fpath, verbose=FLAGS.verbose, attack_log_fpath=attack_log_fpath)
            print (X_test_adv_temp.shape)
            if isinstance(aux_info, float):
                duration+= aux_info
            else:
                duration+= aux_info['duration']
            X_test_adv_temp_list.extend(X_test_adv_temp)
        X_test_adv = np.array(X_test_adv_temp_list)
